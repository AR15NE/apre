...
export class MainLayoutComponent {
  ...
  agents: any[] = []; // Variable to hold the list of agents
  selectedAgent: any;
  ...
  agentPerformanceReports = [
    ...
    { name: 'Call Duration by Agent', url: '/reports/agent-performance/call-duration-by-agent' } // Added for Major Dev Task
  ];
  ...
  constructor(private cookieService: CookieService, private router: Router, private http: HttpClient) {
    ...
    this.userInitial = this.sessionUser ? this.sessionUser.username.slice(0, 1).toUpperCase() : '';
  }

  ngOnInit() {
    this.fetchAgents(); // Fetch the agents when the component initializes
  }

  fetchAgents() {
    this.http.get<any[]>('https://your-api-url.com/api/agents').subscribe((data) => {
      this.agents = data;
    });
  }
