...
// Test the call-duration-by-agent endpoint
it('should fetch agent performance data by call duration', async () => {
  mongo.mockImplementation(async (callback) => {
    const db = {
      collection: jest.fn().mockReturnThis(),
      find: jest.fn().mockReturnValue({
        toArray: jest.fn().mockResolvedValue([
          { agent: 'John Doe', callDuration: 120 },
          { agent: 'Jane Smith', callDuration: 150 }
        ])
      })
    };
    await callback(db);
  ...
  const response = await request(app).get('/api/reports/agent-performance/call-duration-by-agent');
...
// Test the call-duration-by-agent endpoint with no data found
it('should return 200 and an empty array if no data is found', async () => {
  mongo.mockImplementation(async (callback) => {
    const db = {
      collection: jest.fn().mockReturnThis(),
      find: jest.fn().mockReturnValue({
...
// Test the call-duration-by-agent endpoint with an invalid endpoint
it('should return 404 for an invalid endpoint', async () => {
  const response = await request(app).get('/api/reports/agent-performance/invalid-endpoint'); // Send a GET request to an invalid endpoint
  expect(response.status).toBe(404); // Expect a 404 status code
